...
standard manipulator with 2 degrees of freedom. After a certain amount of trainings, the program
runs a test with 500 different angle combinations, calculating the mean error and the standard
deviation. When training is finished, two weight matrices with the trained values are saved in
the same path as the training file and an error function is plotted.
'''
...
for e in range(epochs):
    ...
    trainingangles = np.asarray([0, random.choice(angles), random.choice(angles)])
    ...
    if e % test == 0:
        ...
        for i in range(500):
            ...
            # are randomly chosen.
            testangles = np.asarray([0, random.choice(angles), random.choice(angles)])
            # Set the input to a range between 0.01 and 1.00.
...
# Save the trained weight matrices under '2DOF_wih' and '2DOF_who'.
net.save('cont2DOF')
