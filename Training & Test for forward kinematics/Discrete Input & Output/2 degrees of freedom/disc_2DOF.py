...
standard manipulator with 2 degrees of freedom using discrete inputs and outputs. After a certain
...
for e in range(epochs):
    ...
    trainingangles = random.sample(list(angles),k=2)
    ...
    traininginput[trainingangles[0] + 541]=1.0
    traininginput[trainingangles[1] + 902]=1.0

    # Calculate the correct answers for the chosen angles with homogenous transformation matrices.
    trainresult = manipulator.calculate(0, trainingangles[0], trainingangles[1])
...
# Save the trained weight matrices under '2DOF_x_wih', '2DOF_y_wih', '2DOF_z_wih' and
# '2DOF_x_who', '2DOF_y_who' and '2DOF_z_who'.
net_x.save('2DOF_x')
net_y.save('2DOF_y')
net_z.save('2DOF_z')
