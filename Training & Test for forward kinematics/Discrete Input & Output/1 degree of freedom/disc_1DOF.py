...
standard manipulator with 1 degree of freedom using discrete inputs and outputs. After a certain
...
for e in range(epochs):
    ...
    trainingangles = random.sample(list(angles),k=1)
    ...
    traininginput[180]=1.0
    traininginput[541]=1.0
    traininginput[trainingangles[0] + 902]=1.0

    # Calculate the correct answers for the chosen angles with homogenous transformation matrices.
    trainresult = manipulator.calculate(0, 0, trainingangles)
    ...
    if e % test == 0:
        ...
        for i in range(500):
            ...
            # Depending on the degrees of freedom, 1, 2 or 3 angles are drawn randomly from the list
            # containing all integer numbers from -180 to 180.
            testangles = random.sample(list(angles),k=1)
            # Create the input array with value 0.01 for each element.
            ...
            testinput[180]=1.0
            testinput[541]=1.0
            testinput[testangles[0]+902]=1.0
            ...
            realresult = manipulator.calculate(0, 0, testangles)
...
# Save the trained weight matrices under '1DOF_x_wih', '1DOF_y_wih', '1DOF_z_wih' and
# '1DOF_x_who', '1DOF_y_who' and '1DOF_z_who'.
net_x.save('1DOF_x')
net_y.save('1DOF_y')
net_z.save('1DOF_z')
